...
from pymoo.factory import get_sampling, get_crossover, get_mutation, get_termination,get_problem,get_performance_indicator,get_reference_directions
...
#! set n_partitions list here
n_partitions = list(range(1,20))####!!!!!!!!!!!
...
for problem_name in select_problem_list:
    ...
    for n_partition in n_partitions:
        ...
        ref_dirs = get_reference_directions("das-dennis", p.n_obj, n_partitions= n_partition)
        #algorithm parameters
        algorithm = NSGA3(
        pop_size= 100, #! parameters
        n_offspring = 20,
        ref_dirs=ref_dirs,
        ...
        print('n_partition size',n_partition)
    ...
    # print("Constraint violation: {}".format(res.CV.shape)) 
        ...
        if pf is not None:
            pop_dict[str(n_partition)] = gd.calc(res.F)
        ...
        best_solution_path = path + '/NSGA-II_' + str(problem_name)+'_n_partition_' + str(n_partition) + '_X'
        objective_value_path= path + '/NSGA-II_' + str(problem_name) + '_n_partition_' +str(n_partition) + '_F'
