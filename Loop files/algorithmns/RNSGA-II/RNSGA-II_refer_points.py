...
#!!! set weight parameters here by change number 
refer_point = [(np.random.randint(1,10,4)/10).reshape(2,2) for i in range(100)]

for problem_name in select_problem_list:
    p_dict ={}
    for rp in refer_point:
        print('\n\n')
        print('-'*60)
        print('-'*60)
        ...
        best_solution_path = path + '/RNSGA2_' + str(problem_name)+'_ReferPoint_' + str(rp) + '_X'
        objective_value_path= path + '/RNSGA2_' + str(problem_name) + '_ReferPoint_' +str(rp) + '_F'
#       constraint_viloation_path = path + '/RNSGA2_' + str(problem_name)+ '_ReferPoint_' + str(rp) + '_CV'
